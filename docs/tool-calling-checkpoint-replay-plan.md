...
> 状态：proposed（待评审）· 版本 v18（按十五轮评审：eval 直接用 session.jsonl、不做任何插件；场景聚焦「工具调用不符合预期」不出界；PTC/non-PTC 分标准）
...
模型下一步（system prompt 静态段已提前告知机制与两种 access 约定）：
   ...
   └─ PTC：不注册工具；新 run_code 程序内 tools.read checkpoint → JSON.parse →
              在真实程序文本上 replace → AsyncFunction 构造器执行并 return 其值
...
6. **已知风险（PTC 版，实现期已消解）**：程序参数经 `JSON.stringify` 后，对**文件文本**做「免读直接 edit」存在转义失配风险；实现期定稿改为「`JSON.parse` 后在 `prev.code`（真实程序文本）上 replace，再用 `AsyncFunction` 构造器执行并 return」——匹配串即模型记忆里的原文，无转义问题，顶层 `return`/`await` 语义与原生 run_code 一致（裸 eval 在 strict 模式拒绝 return，实证后弃用）；剩余风险仅剩短片段歧义（JS replace 只替换首个匹配，文案提示换更长唯一片段）与「重试再失败时 checkpoint 存 loader」的间接层（文案已提示）。
...
- 重试流程（实现期定稿：parse + replace + 就地 eval，不引导 `tools.edit`）：模型在新 `run_code` 程序内 `tools.read` 读回 checkpoint（返回值含 `lines`，程序侧拿到的是结构化 value），`JSON.parse` 得到 `{ code, description }`，在**真实程序文本** `prev.code` 上做字面 `replace`（匹配串无 JSON 转义问题，决策点 10 由此消解），再把修正后的程序**作为真实函数执行并返回其值**：`new AsyncFunction("tools", "console", "'use strict';\n" + fixed)` 后 `return await run(tools, console)`——程序内顶层 `return`/`await` 与原生 run_code 语义一致；裸 `eval` 在 strict 模式会拒绝 `return`（实证 `SyntaxError: Illegal return statement`，故采用 AsyncFunction 构造器而非 eval）。若这次重试再失败，新 checkpoint 存的是 loader——其中的 `file_path` 仍指回原程序（文案已提示）。或提取其中的长参数片段直接作为 `tools.<name>(parsed)` 的输入。静态段 B 附完整示例。
...
- **签名（评审修订 + patch 唯一载荷）**：**`{ previous_ordinal?, call_id?, patch }`**——定位参数**二选一、恰填一个**（都填或都不填 → 明确错误结果）；`patch` 是唯一编辑载荷（必填、非空）。每个条目 `{ path, value? | old_string?, new_string?, replace_all? }`，path 定位后**二选一**语义：
  - **`value`**（整值替换）：把 path 处的值整个换掉——任意 JSON 类型，改类型、改数字、换对象都无歧义；`value` 与 `old_string` 都省略 → **删除该字段**（数组下标 splice）；
  - **`old_string + new_string`**（串内替换）：path 处的值必须是**字符串**，在其**解码后的文本**上做字面替换——字符串化 JSON 的值直接用真实引号匹配，转义完全不进入模型视野（解决 long-config 类场景 raw 替换因双重转义而不可用的实测问题）；`replace_all` 默认 false，old_string 出现不止一次时报错并给出次数（模型应加长片段使其唯一或显式 replace_all）。长字符串 value（如 10K plan 改一小节）只发改动片段，不发整值。
  - **路径**：点号段 + `[n]` 数组下标（支持 `.a[0][1]` 链式），从 checkpoint 顶层 key 出发；路径缺失/非法 → 错误结果附顶层键列表。
  - **固有边界**：改字段名（key 改名）不可表达——路径只改值不改键；改类型只能用 value 形式。两者都超出时模型重发新调用。
...
- **单次调用完成编辑+重放**（五轮评审）：模型**不需要**先用内置 `edit` 改 checkpoint——patch 在工具体内应用；因此覆盖时序与重放无冲突（§2.2/§3.2）。持久化的是 `JSON.stringify(patched)`（无换行，与 checkpoint 原始格式一致），重放参数即补丁后的解析对象。
...
- **断点快照 = 一段 session.jsonl 前缀**（fixture），**全部从本机存量 bad case 裁剪**（真实会话日志，解压 .jsonl.zstd 后裁剪到「断点 + turn 边界」，脱敏后入库）：`pnpm crop:real`（维护者运行，不入 commit 门禁）从 ~/.dsh/sessions/ 抽取 5 个真实坏例到 `tests/eval-fixtures-src/real/<name>/`（前缀 zstd 压缩提交，builder 确定性解压进 `tests/eval-fixtures/`）。**断点前缀 = 会话开头到失败步骤最后一条结果之间的全量内容**——所有 turn、所有 thinking、所有工具调用与结果，断点之上零丢失；只丢弃两类无损的 wire 层产物（流式 delta chunk，其内容完整保存在 assistant/message 块里；以及固定版本会话库无法恢复的 steering/message），断点处开放的 step/turn 由 harness 自身的 interrupted-turn repair 在恢复时闭合（与真实崩溃恢复一致）。裁剪器另做三件保真工作：① 删除持久化信封里的 `sourceEventSeqs`（原会话事件 id，裁剪后必非法）并重排 `seq`；② 中性化机器路径（前缀事件、参数、错误文本、工作区内容全量）；③ 工作区快照还原到**断点时刻**的文件状态——若后文重试已把目标内容写进当前文件（fileContains 评分将零成本通过），edit 类按 `new_string→old_string` 反向回滚，不可恢复的退化场景直接抛错拒绝入库，保证评分不可能靠「什么都不做」通过；
...
- **PTC 专属标准**：采用率 = 新 `run_code` 程序内是否读取 checkpoint 路径（`tools.read`/`tools.edit`/bash 访问 `<checkpoint-dir>`，或以旧程序为基础重建）；成功 = 下一次 `run_code` 调用无 `error`；修复粒度是程序级（整程序重建），token 指标侧重「重建步输出 token」；内部子调用失败被捕获的不算断点（§2.2）。
- 轻量任务检查（可选兜底）：场景可自带一个工作区状态校验脚本（参照 limao eval 的 capture/regrade），但不作通用评分要求——本 eval 的边界是「该调用是否被有效重试」，不是整任务质量。
- 每场景输出 JSON（对齐 limao eval 的 batch/report 结构）+ 聚合表（native 与 PTC 分表：token 节省 %、采用率、重试成功率、注入开销）。
...
1. **四段提示词文案**（附录 B）——「免读直接 edit」的措辞已在实现期定稿为 PTC parse-first 路线（决策点 10），其余审阅点：静态段中两种 access 约定（id 与顺序 id 别名）的详细程度、**使用矩阵（上一个→顺序别名；更早→id：失败注入过/成功查 history）**、PTC 版「checkpoint = 整个程序」的引导方式。

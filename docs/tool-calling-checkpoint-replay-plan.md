...
> 状态：proposed（待评审）· 版本 v8（按六轮评审：编号文件改为「顺序 id」软链/快捷方式，唯一真实存储为 id 文件）
...
3. 定位方式（两种 access）：**id**（通知注入 / `tail history.jsonl` 查询，原样回传，无需构造）与**顺序 id**（`1.json`/`2.json`… 别名，指向 id 文件）；PTC 版文案给文件路径；

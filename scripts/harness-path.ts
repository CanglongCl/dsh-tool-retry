...
/**
 * Resolve the Harness root used only by development and self-test launches.
...
export function resolveHarnessRoot(): string {
  ...
  if (harness === undefined || harness === '') {
    throw new Error('dsh-tool-retry: DSH_HARNESS is required for development launches')
  }
  return realpathSync(harness)
}

/**
 * Resolve the built app-owned CLI entry.
...
export function resolveHarnessCli(harnessRoot: string): string {
  ...
  if (!existsSync(cli)) {
    throw new Error(`dsh-tool-retry: Harness CLI is not built at ${cli}; run pnpm setup:harness`)
  }
  return cli
}

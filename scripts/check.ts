...
// The fixture corpora (llm-replay A/B + eval breakpoint prefixes) are
// committed data generated by a builder; the gate regenerates them and
// demands byte-identical output (no hand edits).
run('regenerate fixture corpora', process.execPath, ['--import', 'tsx', join(ROOT, 'scripts/build-fixtures.ts')])
assert(
  'fixture corpora deterministic (replay-fixtures/ + eval-fixtures/ unchanged)',
  () => ['replay-fixtures', 'eval-fixtures']
    .map(dir => spawnSync('git', ['diff', '--exit-code', '--', join(ROOT, 'packages', 'dsh-tool-retry', 'tests', dir)], { encoding: 'utf8' }).status)
    .every(status => status === 0),
  () => 'fixture corpus changed across a regeneration run',
)

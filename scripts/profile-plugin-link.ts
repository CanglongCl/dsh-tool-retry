/** Materialize the source-checkout package under its development alias. */
import {
  existsSync,
  lstatSync,
  mkdirSync,
 ...
 * @param repositoryRoot - dsh-tool-retry repository root.
...
export function materializeProfilePluginLink(
  ...
  if (existsSync(manifestPath)) {
    ...
    if (manifest.dependencies?.[OFFICIAL_PACKAGE_NAME] !== undefined
      ...
      throw new Error(
        `dsh-tool-retry: ${OFFICIAL_PACKAGE_NAME} is already installed in the ${profile} profile; `
        + 'remove it explicitly or use an isolated DSH_HOME before launching the source overlay',
      )
    }
  }
  const target = realpathSync(join(repositoryRoot, 'packages', 'dsh-tool-retry'))
  ...
  if (current !== undefined) {
    if (!current.isSymbolicLink()) {
      throw new Error(`dsh-tool-retry: refusing to replace non-link development package at ${destination}`)
    }

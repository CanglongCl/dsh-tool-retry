...
export function nativeSection(dir: string): string {
  return [
    ...
    '  for that call.',
    'After a failed call, a notice gives you the call id. Instead of re-sending',
    'the whole call, patch ONE field of the saved call and replay it:',
    '',
    '<example name="plan rejected — fix the section the user pointed out and resubmit">',
    '  exit_plan_mode failed: the user rejected the plan ("第 3 节不要用 Python，改成 Rust").',
    '  saved args: { plan: "…## 第 3 节\n继续用 Python 实现…" }',
    `  <tool_call>${REPLAY_TOOL_NAME}({ call_id: "<id>", patch: [`,
    '    { path: ".plan", old_string: "继续用 Python 实现", new_string: "改为 Rust 实现" } ] })</tool_call>',
    '  <tool_result>Replayed exit_plan_mode: plan approved.</tool_result>',
    '</example>',
    '',
    '<example name="edit rejected a stale fragment — fix the fragment I sent">',
    '  edit failed: old_string was not found in "notes.md".',
    '  saved args: { file_path: "notes.md", old_string: "当前版本是 2.0.0。", new_string: "3.0.0。" }',
    `  <tool_call>${REPLAY_TOOL_NAME}({ call_id: "<id>", patch: [`,
    '    { path: ".old_string", old_string: "2.0.0。", new_string: "2.1.0。" } ] })</tool_call>',
    '  <tool_result>Replayed edit: file updated.</tool_result>',
    '</example>',
    '',
    '<example name="wrong type — replace the whole value">',
    '  read failed: offset must be a number (sent "1").',
    '  saved args: { file_path: "notes.md", offset: "1" }',
    `  <tool_call>${REPLAY_TOOL_NAME}({ call_id: "<id>", patch: [`,
    '    { path: ".offset", value: 1 } ] })</tool_call>',
    '  <tool_result>Replayed read: <file content></tool_result>',
    '</example>',
    '',

...
export function nativeSection(dir: string): string {
  return [
    ...
    '',
    '<example name="wrong fragment inside one of my own args">',
    '  eval_edit failed: old_string not found in the file.',
    '  saved args: { file_path: "…", old_string: "…当前版本是 2.0.0。…", new_string: "…" }',
    `  ${REPLAY_TOOL_NAME}({ call_id: "<id>", patch: [`,
    '    { path: ".old_string", old_string: "2.0.0。", new_string: "2.1.0。" } ] })',
    '</example>',
    '',
    '<example name="wrong value deep in an object">',
    '  eval_submit failed: mode must be "prod".',
    '  saved args: { config: { mode: "dev", … } }',
    `  ${REPLAY_TOOL_NAME}({ call_id: "<id>", patch: [`,
    '    { path: ".config.mode", old_string: "dev", new_string: "prod" } ] })',
    '</example>',
    '',
    '<example name="wrong type — replace the whole value">',
    '  eval_target failed: version must be a number (sent "v2").',
    '  saved args: { version: "v2" }',
    `  ${REPLAY_TOOL_NAME}({ call_id: "<id>", patch: [`,
